...
if analysis_option == "Artist Discography over Time":
  ...
  top_5 = [artist for artist in df['Artists'].unique().tolist() if len(artist) < 15]

  top_5_artists = sorted(top_5, key=lambda x: df['Artists'].value_counts()[x])
  ...
  artists = [artist for artist in df['Artists'].unique().tolist() if len(artist) < 15]

  unique_artists = sorted(artists, key=lambda x: df['Artists'].value_counts()[x])
